...
public interface InsurancePolicyRepository extends JpaRepository<InsurancePolicy, Long> {
    ...
    @Query("SELECT p FROM InsurancePolicy p WHERE p.endDate <= :today AND (p.logged IS NULL OR p.logged = false)")
    List<InsurancePolicy> findExpiredNotLogged(@Param("today") LocalDate today);
}